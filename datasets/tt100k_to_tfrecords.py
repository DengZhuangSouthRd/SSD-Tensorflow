...
DIRECTORY_ANNOTATIONS = 'annotation/'
DIRECTORY_IMAGES = 'src/'
...
def _process_image(directory, name):
    ...
    with open(filename, "r") as handler:
        anno_targets = json.load(handler)
        # obj [xmin, ymin, xmax, ymax]
        for obj in anno_targets:
            bboxes.append(
                (obj[1] / shape[0], obj[0] / shape[1],
                obj[3] / shape[0], obj[2] / shape[1]))
            difficult.append(0)
            truncated.append(0)
            labels.append(obj[4])
            labels_text.append(str(obj[4]))
...
def run(dataset_dir, output_dir, name='tt100k', shuffling=False):
